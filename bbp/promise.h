...
// argument satisfying 'Callable<Types...>' and their second argument satisfying
...
public:
  ...
  // created promise with a fulfillment value in case this promise is rejected.
  ...
  //    'promise<T1,T2,...>'.
  // 3. If the return type of 'fulfilledCont' is 'promise<T1,T2,...>' for some
  //    types 'T1,T2,...', the result of this function will be of type
  //    'promise<T1,T2,...>'.
  ...
  then(FulfilledCont fulfilledCont, RejectedCont rejectedCont) // Two-argument version of case #4
      requires(
    ...
    if (std::vector<std::pair<std::function<void(Types...)>,
      ...
      return promise<U>(
          [ t, fulfilledCont = std::move(fulfilledCont) ](auto fulfill, auto reject) mutable {
            try {
      ...
      return promise<U>(
          [&e, rejectedCont = std::move(rejectedCont) ](auto fulfill, auto reject) mutable {
            try {
  ...
  template <Callable<Types...> FulfilledCont,
            Callable<std::exception_ptr> RejectedCont>
  requires(
      std::is_void<typename std::result_of<FulfilledCont(Types...)>::type>::value
          &&std::is_same<typename std::result_of<FulfilledCont(Types...)>::type,
    ...
    if (std::vector<std::pair<std::function<void(Types...)>,
      ...
      // probably be moved in. Maybe some tests with weird functions
      // should be used.
      return promise<>([waitingFunctions, f, ef](auto fulfill, auto reject) {
        waitingFunctions->emplace_back(
  ...
  template <Callable<Types...> FulfilledCont>
  requires !std::is_void<typename std::result_of<FulfilledCont(Types...)>::type>::
      value promise<typename std::result_of<FulfilledCont(Types...)>::type>
      then(FulfilledCont f) {
    ...
    if (std::vector<std::pair<std::function<void(Types...)>,
            ...
            *const waitingFunctions =
                bbp::get_if<waiting_state>(d_data->d_state)) {
      return promise<U>([waitingFunctions, f](auto fulfill, auto reject) {
        waitingFunctions->emplace_back(
            [f, fulfill, reject](Types... t) {
              try {
                fulfill(f(t...));
