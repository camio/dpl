...
struct wrap_call {
    ...
    template <class... Xs>
    constexpr auto operator()(Xs&&... xs) const & noexcept(noexcept(
        f(std::forward<Xs>(xs)...))) -> decltype(f(std::forward<Xs>(xs)...))
    {
...
struct function_member_wrapper {
    ...
    constexpr function_member_wrapper(type fct)
    : f(fct)
    {
    }
...
#define WRAP_CALL_FOR_MEMBER_FUNCTION(cv_ref_qual)                            \
    ...
    struct wrap_call<R (C::*)(Args...) cv_ref_qual>                           \
        : function_member_wrapper<R (C::*)(Args...) cv_ref_qual,              \
                                  C,                                          \
                                  R,                                          \
                                  Args...> {                                  \
        using base = function_member_wrapper<R (C::*)(Args...) cv_ref_qual,   \
                                             C,                               \
                                             R,                               \
                                             Args...>;                        \
        using base::base;                                                     \
...
struct overload_storage
    : std::conditional<std::is_class<F>::value && !std::is_final<F>::value,
                       F,
                       wrap_call<F> > {
};
...
{
    return detail::
        explicit_overloader<R, std::decay_t<F1>, std::decay_t<Fs>...>(
