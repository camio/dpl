...
namespace dplm20 {
namespace detail {
template <class F> struct wrap_call {
  using type = F;
  type f;
  ...
  template <
      ...
      std::enable_if_t<!std::is_base_of<wrap_call, std::decay_t<U>>::value
                           ...
                           && std::is_constructible<F, U>::value,
                       bool> = true>
  ...
  template <class... Xs>
  constexpr auto
  operator()(Xs &&... xs) const &noexcept(noexcept(f(std::forward<Xs>(xs)...)))
      -> decltype(f(std::forward<Xs>(xs)...)) {
    return f(std::forward<Xs>(xs)...);
  }
  template <class... Xs>
  constexpr auto operator()(Xs &&... xs) const &&noexcept(
      noexcept(std::move(f)(std::forward<Xs>(xs)...)))
...
template <typename R, typename... Args> struct wrap_call<R (*)(Args...)> {
  using type = R (*)(Args...);
  type f;
...
struct function_member_wrapper {
  ...
  template <
      ...
      std::enable_if_t<std::is_base_of<Clazz, std::decay_t<U>>::value &&
                           std::is_convertible<decltype((std::declval<U>().*f)(
                                                   std::declval<Args>()...)),
                                               R>::value,
                       bool> = true>
  ...
  template <
      typename U,
      std::enable_if_t<std::is_convertible<decltype((std::declval<U &>().*f)(
                                               std::declval<Args>()...)),
                                           R>::value,
                       bool> = true>
  ...
  template <typename U,
            std::enable_if_t<
                !std::is_base_of<Clazz, std::decay_t<U>>::value &&
                    std::is_convertible<decltype(((*std::declval<U>()).*
                                                  f)(std::declval<Args>()...)),
                                        R>::value,
                bool> = true>
...
#define WRAP_CALL_FOR_MEMBER_FUNCTION(cv_ref_qual)                             \
  ...
  struct wrap_call<R (C::*)(Args...) cv_ref_qual>                              \
      : function_member_wrapper<R (C::*)(Args...) cv_ref_qual, C, R,           \
                                Args...> {                                     \
    using base =                                                               \
...
/* If F is not final class make it direct base of the overload, otherwise wrap
   in into a call.
   Notice that both F and wrap_call<F> will be constructible from same object
   (prefect forwarding constructor) */
template <typename F>
struct overload_storage
    : std::conditional<std::is_class<F>::value && !std::is_final<F>::value, F,
                       wrap_call<F>> {};

template <typename F>
using overload_storage_t = typename overload_storage<F>::type;

template <class F, class... Fs> class overloader;

template <typename F1> class overloader<F1> : overload_storage_t<F1> {
  using base1 = overload_storage_t<F1>;

public:
  template <typename U1>
  constexpr explicit overloader(U1 &&fct1) : base1(std::forward<U1>(fct1)) {}

  using base1::operator();
};
template <typename F1, typename F2, class... Fs>
class overloader<F1, F2, Fs...> : overload_storage_t<F1>,
                                  overloader<F2, Fs...> {
  using base1 = overload_storage_t<F1>;
...
template <class R, class... Fs> struct explicit_overloader : overloader<Fs...> {
  using result_type = R;
...
constexpr auto overload(F1 &&f1, Fs &&... fs) {
  return detail::overloader<std::decay_t<F1>, std::decay_t<Fs>...>(
      std::forward<F1>(f1), std::forward<Fs>(fs)...);
}
